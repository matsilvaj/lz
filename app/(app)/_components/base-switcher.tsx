...
import { switchBaseAction } from "../bases/actions";
...
export function BaseSwitcher({ activeBase, bases }: BaseSwitcherProps) {
        ...
        <div className="absolute right-0 top-full z-20 mt-2 min-w-56 rounded-2xl border border-neutral-200 bg-white p-2 shadow-sm">
          ...
          <div className="space-y-1">
            {bases.map((base) => {
              ...
              return (
                <form
                  action={switchBaseAction.bind(null, base.id, pathname)}
                  key={base.id}
                >
                  <button
