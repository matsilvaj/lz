...
function buildOperationDateFromIsoDate(value = "") {
...
function formatOperationDateLabel(value = "") {
  const parts = parseOperationDateParts(value);

  if (!parts) {
    return value || "Dia";
  }

  return `${padNumber(parts.day)}/${padNumber(parts.month)}/${parts.year}`;
}
...
function buildCustomDashboardPeriodOption(requestedPeriodId = "") {
  const [type, first, second] = String(requestedPeriodId ?? "").split(":");

  if (type === "day") {
    const operationDate = buildOperationDateFromIsoDate(first);

    if (!operationDate) {
      return null;
    }

    const key = String(parseOperationDate(operationDate));
    ...
    return {
      ...
      helper: "Dia",
      id: `day:${first}`,
      label: formatOperationDateLabel(operationDate),
  ...
  if (type === "range") {
    const startDate = buildOperationDateFromIsoDate(first);
    const endDate = buildOperationDateFromIsoDate(second);

    if (!startDate || !endDate) {
      return null;
    }

    const startKey = parseOperationDate(startDate);
    const endKey = parseOperationDate(endDate);
    ...
    return {
      ...
      label:
        startDate === endDate
          ? formatOperationDateLabel(startDate)
          : `${formatOperationDateLabel(startDate)} a ${formatOperationDateLabel(endDate)}`,
      startDate,
...
function resolveDashboardPeriodOption(periodOptions, requestedPeriodId, referenceMonth) {
  ...
  const customPeriod = buildCustomDashboardPeriodOption(requested);
...
function buildDashboardTimeline(period, referenceMonths, seriesRows, convertedRows) {
  if (["day", "days", "range"].includes(period.type)) {
...
// Filtro "Parceiro": "me" e/ou ids de parceiros, em ordem estável (faz parte da chave do cache).
export function normalizePartnerFilterValues(values) {
  const tokens = (Array.isArray(values) ? values : String(values ?? "").split(","))
    .map((value) => String(value ?? "").trim())
    .filter((value) => /^(me|\d{1,12})$/u.test(value));

  return [...new Set(tokens)].sort().slice(0, 50);
}
...
async function getDashboardDataUncached(
  ...
  const selectedPartners = normalizePartnerFilterValues(partnersKey);
  const [
    historyMonths,
    freebetsSummary,
    pendingProcedures,
    ...
    repository.getFreebetsSummary(userId, baseId, selectedPartners),
    repository.getPendingProceduresSummary(userId, baseId, selectedPartners),
    repository.listPartners(userId),
  ]);
  ...
  const [dashboardStats, convertedFreebetProfit] = await Promise.all([
    ...
    repository.getConvertedFreebetProfitByPeriod(activePeriod, userId, baseId, selectedPartners),
  ]);
...
export async function getDashboardData(userId, baseId, selectedPeriodId = "", partners = []) {
  return getCachedDashboardData(
    userId,
    baseId,
    buildCurrentReferenceMonth(),
    normalizeReferenceMonth(selectedPeriodId),
    normalizePartnerFilterValues(partners).join(","),
  );
}

export async function getProceduresPageData(userId, baseId, filters = {}) {
  const repository = getProceduresRepository();
  const [proceduresResult, bookmakers, partners] = await Promise.all([
...
async function getFreebetsPageDataUncached(userId, baseId) {
  const repository = getProceduresRepository();
  const [activeFreebets, convertedFreebets, bookmakers, summary, partners] = await Promise.all([
...
async function getBookmakersPageDataUncached(userId, baseId) {
  const repository = getProceduresRepository();
  const [bookmakers, availableBookmakers, notes, partnerBookmakers, partners] = await Promise.all([
    repository.listBookmakersWithBalance(userId, baseId),
    getBookmakersCatalog(),
    repository.getBookmakersNotes(userId, baseId),
    repository.listPartnerBookmakers(userId, baseId),
    repository.listPartners(userId),
  ]);
...
async function getHistoryPageDataUncached(
  userId,
  baseId,
  selectedPeriodId = "",
  partnersKey = "",
  ...
  const selectedHouses = housesKey ? housesKey.split("|").filter(Boolean) : [];
  const selectedStatuses = statusesKey ? statusesKey.split("|").filter(Boolean) : [];
  const selectedMultiples = multiplesKey ? multiplesKey.split("|").filter(Boolean) : [];
  const [historyMonths, partners, bookmakers] = await Promise.all([
    repository.listHistoryMonths(userId, baseId, selectedPartners),
    repository.listPartners(userId),
    getBookmakersCatalog(),
  ]);
  const months = buildHistoryMonthOptions(historyMonths);
  const periodOptions = buildDashboardPeriodOptions(months);
  const activePeriod = resolveDashboardPeriodOption(
    periodOptions,
    selectedPeriodId,
    months[0]?.value ?? buildCurrentReferenceMonth(),
  );
  const operations = await repository.listHistoryOperations(
    activePeriod,
    userId,
    baseId,
    {
      houses: selectedHouses,
      multiples: selectedMultiples,
      onlyFavorites,
...
export async function getHistoryPageData(
  ...
  // As listas fazem parte da chave do cache: ordem estável e sem repetição.
  const toKey = (values) => [...new Set(values)].sort().join("|");

  return getCachedHistoryPageData(
    userId,
    baseId,
    selectedPeriodId,
    normalizePartnerFilterValues(partners).join(","),
