...
async function getHistoryPageDataUncached(userId, baseId, selectedMonth = "", partnersKey = "") {
  const repository = getProceduresRepository();
  const selectedPartners = normalizePartnerFilterValues(partnersKey);
  const [historyMonths, partners] = await Promise.all([
  ...
  const [operations, bookmakers] = await Promise.all([
    months.length > 0
      ? repository.listHistoryOperationsByMonth(activeMonth, userId, baseId, selectedPartners)
      : [],
    getBookmakersCatalog(),
  ]);
...
export async function getHistoryPageData(userId, baseId, selectedMonth = "", partners = []) {
