...
export async function getDashboardData(userId) {
  ...
  const [procedures, activeFreebets, convertedFreebets] = await Promise.all([
    repository.listProcedures(userId),
    repository.listActiveFreebets(userId),
    repository.listConvertedFreebets(userId),
  ]);
...
export async function getProceduresPageData(userId) {
  const repository = getProceduresRepository();
...
export async function getFreebetsPageData(userId) {
  const repository = getProceduresRepository();
...
export async function getBookmakersPageData(userId) {
  const repository = getProceduresRepository();
  const [bookmakers, availableBookmakers, notes] = await Promise.all([
    repository.listBookmakersWithBalance(userId),
    repository.listBookmakers(),
    repository.getBookmakersNotes(userId),
  ]);
...
export async function getHistoryPageData(userId) {
  const repository = getProceduresRepository();
  const procedures = await repository.listProcedures(userId);
