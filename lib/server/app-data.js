...
function getReferenceMonthTimelineDayLimit(
  referenceMonth,
  { procedures = [], convertedFreebets = [], now = new Date() } = {},
) {
  const parts = parseReferenceMonthParts(referenceMonth);
  ...
  const procedureDays = (procedures ?? [])
    .map((procedure) => parseOperationDateParts(procedure?.data_operacao))
    .filter(isSameMonth)
    .map((dateParts) => dateParts.day);

  const convertedFreebetDays = (convertedFreebets ?? [])
    .map((item) => parseOperationDateParts(item?.data_conversao))
    .filter(isSameMonth)
    .map((dateParts) => dateParts.day);

  return Math.min(
    Math.max(now.getDate(), ...procedureDays, ...convertedFreebetDays),
    monthTotalDays,
  );
}
...
function fillMissingActiveFreebetDates(activeFreebets, procedures) {
  const procedureDateById = new Map(
    (procedures ?? [])
      .map((procedure) => [
        toNumber(procedure?.id, Number.NaN),
        String(procedure?.data_operacao ?? "").trim(),
      ])
      .filter(([id, date]) => Number.isFinite(id) && date),
  );

  return {
    pendentes_confirmacao: (activeFreebets?.pendentes_confirmacao ?? []).map((item) => ({
      ...item,
      data:
        String(item?.data ?? "").trim() ||
        procedureDateById.get(toNumber(item?.id, Number.NaN)) ||
        "",
    })),
    agrupadas_convertiveis: (activeFreebets?.agrupadas_convertiveis ?? []).map((item) => ({
      ...item,
      data:
        String(item?.data ?? "").trim() ||
        (item?.ids ?? [])
          .map((id) => procedureDateById.get(toNumber(id, Number.NaN)))
          .find(Boolean) ||
        "",
    })),
  };
...
function buildDailyOperationSeries(procedures, referenceMonth, timelineDays) {
  const groups = new Map();
...
function buildDailyCollectedFreebetSeries(procedures, referenceMonth, timelineDays) {
  const groups = new Map();
...
function buildDailyConvertedFreebetProfitSeries(
  convertedFreebets,
  referenceMonth,
  timelineDays,
) {
  ...
  for (const item of convertedFreebets ?? []) {
    ...
    const current = groups.get(conversionDate) ?? {
      label: conversionDate.slice(0, 2),
      fullLabel: conversionDate,
      dateKey,
      value: 0,
      count: 0,
    };
...
function buildFreebetMetrics(procedures, convertedFreebets, referenceMonth, timelineDays) {
  return {
    collectedDaily: buildDailyCollectedFreebetSeries(
      procedures,
      referenceMonth,
      timelineDays,
    ),
...
function buildDashboardView(procedures, referenceMonth, timelineDays) {
  const monthlyProcedures = filterProceduresByReferenceMonth(procedures, referenceMonth);
  const dailySeries = buildDailyOperationSeries(
    monthlyProcedures,
    referenceMonth,
    timelineDays,
  );
...
export async function getDashboardData(userId, baseId) {
  ...
  const timelineDays = getReferenceMonthTimelineDayLimit(referenceMonth, {
    procedures,
    convertedFreebets,
  });
  ...
  const views = Object.fromEntries(
    procedureFilters.map((filter) => [
      filter,
      buildDashboardView(
        filterProceduresByType(procedures, filter),
        referenceMonth,
        timelineDays,
      ),
    ]),
...
export async function getFreebetsPageData(userId, baseId) {
  const repository = getProceduresRepository();
  const [activeFreebets, convertedFreebets, bookmakers, procedures] = await Promise.all([
    repository.listActiveFreebets(userId, baseId),
    repository.listConvertedFreebets(userId, baseId),
    repository.listBookmakers(),
    repository.listProcedures(userId, baseId),
  ]);
  const activeFreebetsWithDates = fillMissingActiveFreebetDates(activeFreebets, procedures);

  return {
    pendingConfirmation: activeFreebetsWithDates.pendentes_confirmacao ?? [],
    convertibleGroups: activeFreebetsWithDates.agrupadas_convertiveis ?? [],
    convertedHistory: convertedFreebets.slice(0, 20),
    bookmakers,
    summary: buildFreebetsSummary(activeFreebetsWithDates, convertedFreebets),
  };
}
