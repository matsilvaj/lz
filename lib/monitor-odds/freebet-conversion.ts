...
function indexConversionOdds(
  ...
  // Limite aplicado por categoria: odds com PA costumam ser menores e, sem separar,
  // nunca entrariam entre as melhores.
  const toList = (group: Map<string, DuploOddItem>, limit: number) => {
    const sorted = Array.from(group.values()).sort(compareByOdd);
